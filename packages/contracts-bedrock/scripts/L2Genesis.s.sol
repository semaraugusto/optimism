...
contract L2Genesis is Deployer {
  ...
  function newArtifactDependencies()
    internal
    view
    returns (L1Dependencies memory l1Dependencies_)
  {
  ...
  /// @notice This is used by op-e2e to have a version of the L2 allocs for each upgrade.
  function newRunWithAllUpgrades() public {
    newRunWithOptions(OutputMode.ALL, LATEST_FORK, newArtifactDependencies());
  }
  ...
  function setL2ERC721Bridge(address payable _l1ERC721BridgeProxy) public {
    ...
    L2ERC721Bridge(Predeploys.L2_ERC721_BRIDGE).initialize({
      _l1ERC721Bridge: payable(_l1ERC721BridgeProxy)
    });
  }
  ...
  function setSequencerFeeVault() public {
    SequencerFeeVault vault = new SequencerFeeVault({
      ...
      _withdrawalNetwork: FeeVault.WithdrawalNetwork(
  ...
  function setGovernanceToken() public {
    ...
    vm.store(
      Predeploys.GOVERNANCE_TOKEN,
      _ownerSlot,
      bytes32(uint256(uint160(cfg.governanceTokenOwner())))
    );
  ...
  function fundDevAccounts() internal {
    for (uint256 i; i < devAccounts.length; i++) {
      console.log(
        'Funding dev account %s with %s ETH',
        devAccounts[i],
        DEV_ACCOUNT_FUND_AMT / 1e18
      );
